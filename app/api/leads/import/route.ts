  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (leadData.name)
          leadData.name = sanitizeCSVValue(String(leadData.name));
        if (leadData.email)
          leadData.email = sanitizeCSVValue(String(leadData.email));
        ...
        if (leadData.company)
          leadData.company = sanitizeCSVValue(String(leadData.company));
        if (leadData.source)
          leadData.source = sanitizeCSVValue(String(leadData.source));
